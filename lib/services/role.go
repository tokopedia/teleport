...
type Role interface {
	...
	// CheckRules checks the role's allow and deny rules
	CheckRules() error
...
// CheckRules validates the role's allow and deny rules.
//
// This method intentionally has been made separate from CheckAndSetDefaults
// to avoid validation errors when dealing with roles which may have been
// extended with non standard rules (for example, generated by external systems
// built on top of Teleport) but to have ability to check roles created by a
// user.
func (r *RoleV3) CheckRules() error {
	for i := range r.Spec.Allow.Rules {
		err := r.Spec.Allow.Rules[i].CheckAndSetDefaults()
		if err != nil {
			return trace.BadParameter(
				"failed to process 'allow' rule %v: %v", i, err)
		}
	}
	for i := range r.Spec.Deny.Rules {
		...
		if err != nil {
			return trace.BadParameter(
				"failed to process 'deny' rule %v: %v", i, err)
		}
	}
	return nil
}
...
// CheckRules is no-op for RoleV2 which didn't have rules
func (r *RoleV2) CheckRules() error {
	return nil
}
